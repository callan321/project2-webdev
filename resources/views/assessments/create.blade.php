...
@section('main')
    <div class="max-w-4xl mx-auto py-10">
        <h1 class="text-3xl font-bold mb-6 text-center">Create New Assessment for {{ $course->name }} ({{ $course->code }})</h1>
        ...
        <form action="{{ route('assessments.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="instruction" id="instruction" rows="4" class="mt-1 block w-full" required>{{ old('instruction') }}</textarea>
            </div>

            <!-- Max Score -->
            <div class="mb-4">
                <label for="max_score" class="block text-sm font-medium text-gray-700">Max Score</label>
                <input type="number" name="max_score" id="max_score" class="mt-1 block w-full" min="1" max="100" value="{{ old('max_score') }}" required>
            </div>

            <!-- Due Date -->
            <div class="mb-4">
                <label for="due_date" class="block text-sm font-medium text-gray-700">Due Date</label>
                <input type="date" name="due_date" id="due_date" class="mt-1 block w-full" value="{{ old('due_date') }}" required>
            </div>

            <!-- Due Time -->
            <div class="mb-4">
                <label for="due_time" class="block text-sm font-medium text-gray-700">Due Time</label>
                <input type="time" name="due_time" id="due_time" class="mt-1 block w-full" value="{{ old('due_time') }}" required>
            </div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded">Create Assessment</button>
            </div>
        </form>
    </div>
@endsection
